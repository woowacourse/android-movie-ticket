...
inline fun SeatSelectionResult.onSuccess(block: (SeatBoard, Seat) -> Unit): SeatSelectionResult {
    if (this is SeatSelectionResult.Success) block(board, updatedSeat)
    return this
}

inline fun SeatSelectionResult.onFailure(block: () -> Unit): SeatSelectionResult {
    if (this is SeatSelectionResult.Failure) block()
    return this
}

fun SeatSelectionResult.isSuccess(): Boolean = this is SeatSelectionResult.Success

fun SeatSelectionResult.isFailure(): Boolean = this is SeatSelectionResult.Failure
