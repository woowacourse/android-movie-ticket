...
internal class ReservationTest {
    ...
    fun `예매를 생성할 때 어떠한 좌석도 예매하지 않으면 에러가 발생한다`() {
        ...
        assertThatIllegalArgumentException()
            .isThrownBy { Reservation(LocalDateTime.now(), theater, emptySeatPoints) }
    ...
    fun `예매를 생성할 때 예매 좌석이 극장 내에 존재하지 않으면 에러가 발생한다`() {
        ...
        assertThatIllegalArgumentException()
            .isThrownBy { Reservation(LocalDateTime.now(), theater, listOf(notExistSeatPoint)) }
    ...
    fun `예매 요금은 각 예매 좌석 요금에 할인을 적용한 금액의 총 합이다`() {
        ...
        val reservation = Reservation(screeningDateTime, theater, seatPoints)
