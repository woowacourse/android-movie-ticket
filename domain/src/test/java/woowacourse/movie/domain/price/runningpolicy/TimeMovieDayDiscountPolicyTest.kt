package woowacourse.movie.domain.price.runningpolicy
...
class TimeMovieDayDiscountPolicyTest {
    ...
    fun `11시이전(조조할인)만 적용된다면 시간할인 정책만 담긴 리스트를 반환한다`() {
        ...
        assertEquals(
            ...
            listOf(woowacourse.movie.domain.price.discount.partialpolicy.EarlyMorningLateNightDiscount()).map { it.javaClass }
        )
    }
    ...
    fun `10일무비데이 할인만 적용된다면 무비데이 할인 정책만 담긴 리스트를 반환한다`() {
        ...
        assertEquals(
            policies.getDiscountPolicies().map { it.javaClass },
            listOf(woowacourse.movie.domain.price.discount.partialpolicy.MovieDayDiscount()).map { it.javaClass }
        )
    }
