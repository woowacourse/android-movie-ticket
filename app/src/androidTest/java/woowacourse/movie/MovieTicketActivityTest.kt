...
class MovieTicketActivityTest {
    @get:Rule
    val movieTicketRule = ActivityScenarioRule(MovieTicketActivity::class.java)

    @Test
    fun `초기화면에는_영화_목록이_있다`() {
        onView(withId(R.id.movies)).check(matches(isDisplayed()))
    }

    @Test
    fun 영화_목록_아이템에는_영화_타이틀이_해리포터와_마법사의_돌이다() {
        onView(withId(R.id.movie_title)).check(matches(withText(HARRY_POTTER_TITLE)))
    }

    @Test
    fun 영화_목록_아이템에는_영화_상영일이_있다() {
        onView(withId(R.id.movie_date)).check(matches(withText(HARRY_POTTER_DATE)))
    }

    @Test
    fun 영화_목록_아이템에는_영화_러닝타임이_있다() {
        onView(withId(R.id.movie_running_time)).check(matches(withText(HARRY_POTTER_RUNNING_TIME)))
    }
    ...
    @Test
    fun 화면을_스크롤할_수_있다() {
        onView(withId(R.id.movies)).perform(swipeUp())
    }
}
