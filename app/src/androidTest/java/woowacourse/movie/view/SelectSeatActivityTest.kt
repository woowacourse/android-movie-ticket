...
class SelectSeatActivityTest {
    ...
    @Test
    fun `영화_제목이_표시된다`() {
        onView(withId(R.id.tv_select_seat_movie_title))
    ...
    @Test
    fun `좌석_테이블이_표시된다`() {
        onView(withId(R.id.tl_seat))
            .check(matches(isDisplayed()))
    }

    @Test
    fun `초기_티켓_가격이_0원으로_표시된다`() {
        onView(withId(R.id.tv_select_seat_selected_ticket_price))
            .check(matches(isDisplayed()))
    ...
    fun `좌석을_선택하면_배경색이_변경된다`() {
        ...
        onView(withId(R.id.seat_A1))
            .check(matches(hasBackgroundColor(R.color.white)))

        // A1 좌석 선택
        onView(withId(R.id.seat_A1))
            .perform(click())
            .check(matches(hasBackgroundColor(R.color.selected_seat_background)))
    ...
    fun `최대_인원을_초과하여_좌석을_선택하면_오류_메시지가_표시된다`() {
        // 좌석 2개 선택 (티켓은 2장)
        onView(withId(R.id.seat_A1)).perform(click())
        onView(withId(R.id.seat_A2)).perform(click())

        // 추가 좌석 선택 시도
        onView(withId(R.id.seat_A3)).perform(click())
    ...
    fun `전체_인원만큼_좌석_선택시_확인_버튼이_활성화된다`() {
        ...
        // 확인 버튼 활성화(보라색 배경) 확인
        onView(withId(R.id.tv_select_seat_confirm))
            .check(matches(hasBackgroundColor(R.color.purple_500)))
    }
    ...
    fun `인원_미만_좌석_선택시_확인_버튼이_비활성화된다`() {
        ...
        // 확인 버튼 비활성화(회색 배경) 확인
        onView(withId(R.id.tv_select_seat_confirm))
            .check(matches(hasBackgroundColor(R.color.disabled_btn_color)))
    }
    ...
    fun `좌석_선택_완료_후_확인_버튼_클릭시_다이얼로그가_표시된다`() {
        ...
        // 확인 버튼 클릭
        onView(withId(R.id.tv_select_seat_confirm)).perform(click())
