...
class MovieViewHolder(val view: View) {
    ...
    fun bind(
        movie: MovieUiModel,
        onReservationClick: (MovieUiModel) -> Unit,
    ) {
        val formatter = DateTimeFormatter.ofPattern(view.context.getString(R.string.date_format))
        val startDate = movie.startDate.format(formatter)
        val endDate = movie.endDate.format(formatter)

        poster.setImageResource(movie.poster)
        title.text = movie.title
        screeningDate.text = view.context.getString(R.string.screening_dates_format, startDate, endDate)
        runningTime.text = view.context.getString(R.string.running_type_format, movie.runningTime)
