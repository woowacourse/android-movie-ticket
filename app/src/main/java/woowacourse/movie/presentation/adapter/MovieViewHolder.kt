...
class MovieViewHolder(itemView: View, private val buttonClickedListener: (Int) -> Unit) :
    ...
    init {
        reserveButton.setOnClickListener {
            buttonClickedListener(adapterPosition)
        }
    }
    ...
    fun bind(movie: MovieUiModel) {
        ...
        screeningDate.text = itemView.context.getString(
            R.string.screening_date_format,
            movie.screeningStartDate,
            movie.screeningEndDate,
        )
        runningTime.text =
            itemView.context.getString(R.string.running_time_format, movie.runningTime)
    }
}
