...
class MovieDetailPresenterTest {
    ...
    fun `정상적인 인덱스의 loadMovie를 호출하여 뷰와 어댑터를 초기화한다`() {
        ...
        every { detailContractView.updateDateList(any(), any()) } just runs
        every { detailContractView.updateTimeList(any(), any()) } just runs
        every { detailContractView.onError(any()) } just runs

        detailPresenter.loadMovie(0)

        verify { detailContractView.onUpdateView(any()) }
        verify { detailContractView.updateDateList(any(), any()) }
        verify { detailContractView.updateTimeList(any(), any()) }
    ...
    fun `데이터가 없는 인덱스의 loadMovie를 호출하여 뷰와 어댑터를 초기화한다`() {
        ...
        verify(exactly = 0) { detailContractView.updateDateList(any(), any()) }
        verify(exactly = 0) { detailContractView.updateTimeList(any(), any()) }
