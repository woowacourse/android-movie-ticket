...
class MovieDateTest {
    ...
    fun `영화_상영일의_시작날짜는_종료날짜보다_이르거나_같아야한다`() {
        ...
        assertDoesNotThrow { MovieDate(startDate, endDate) }
    }
    ...
    fun `영화_상영일의_시작날짜는_종료날짜보다_늦을_수_없다`() {
        ...
        val endDate = LocalDate("2024.04.01")
        assertThrows<IllegalArgumentException> { MovieDate(startDate, endDate) }
    }
    ...
    fun `영화_상영일은_시작날짜부터_종료날짜까지의_날짜_범위_리스트를_갖는다`() {
        ...
        val actual = MovieDate(startDate, endDate).getDatesBetweenStartAndEnd()
